...
export default function ProductDetailPage() {
  ...
  const activeVariant = hasVariants ? variants.find(v => v.id === selectedVariant) : null;
  const isSoldOut = hasVariants ? (activeVariant?.status === "sold_out") : product.status === "sold_out";
  const isSeasonal = product.status === "seasonal";
  const basePrice = activeVariant ? activeVariant.price : product.price;

  // Current display image (variant image overrides if set)
  const displayImages = activeVariant?.image ? [activeVariant.image, ...images.filter(i => i !== activeVariant.image)] : images;
  ...
  const validate = () => {
    const newErrors = {};
    if (hasVariants && !selectedVariant) newErrors._variant = "Please select a variant";
    if (hasVariants && activeVariant?.status === "sold_out") newErrors._variant = "Selected variant is sold out";
    for (const group of modifierGroups) {
      if (group.type === "required") {
        const sel = selections[group.id] || [];
        if (sel.length < (group.min_selections || 1)) newErrors[group.id] = `Please select ${group.min_selections || 1} option`;
      }
    }
    setErrors(newErrors);
  ...
  const handleAddToOrder = () => {
    if (product.status === "sold_out" && !hasVariants) return;
    if (!validate()) return;
    const modifiers = [];
    for (const group of modifierGroups) {
      const sel = selections[group.id] || [];
  ...
  const prevImg = () => setActiveImg(p => (p === 0 ? displayImages.length - 1 : p - 1));
  const nextImg = () => setActiveImg(p => (p === displayImages.length - 1 ? 0 : p + 1));

  const allSoldOut = hasVariants && variants.every(v => v.status === "sold_out");
  const effectivelySoldOut = (!hasVariants && product.status === "sold_out") || allSoldOut;

  const statusBadge = () => {
    if (effectivelySoldOut) return <span data-testid="status-badge" className="px-3 py-1 bg-red-600 text-white text-[10px] font-body font-bold uppercase tracking-wider rounded">Sold Out</span>;
  ...
  return (
    <div data-testid="product-detail-page" className={`min-h-screen ${effectivelySoldOut ? "" : "pb-24 sm:pb-0"}`}>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
        ...
        <div className="flex items-baseline gap-2 mt-1">
          ...
          {(modifierTotal > 0 || (hasVariants && activeVariant)) && (
            <span data-testid="price-breakdown" className="font-body text-xs text-brand-text-secondary">
              {hasVariants && activeVariant && `${activeVariant.name}`}
        ...
        {effectivelySoldOut && !hasVariants && (
        ...
        {/* ─── Variant Selector ─── */}
        {hasVariants && (
          <div data-testid="variant-selector" className={`mt-6 sm:mt-8 bg-brand-surface border rounded-xl overflow-hidden ${errors._variant ? "border-red-400" : "border-brand-border"}`}>
            <div className={`px-4 py-3 flex items-center justify-between ${errors._variant ? "bg-red-50" : "bg-brand-bg/50"}`}>
              <div className="flex items-center gap-2">
                <Package size={16} className="text-brand-text-secondary" />
                <h2 className="font-heading text-base sm:text-lg font-bold text-brand-text">Select Variant</h2>
                <span className="px-2 py-0.5 bg-brand-primary text-white text-[9px] font-body font-bold uppercase tracking-wider rounded">Required</span>
              </div>
              {activeVariant?.stock >= 0 && activeVariant.status !== "sold_out" && (
                <span className="font-body text-[10px] text-brand-text-secondary">{activeVariant.stock} left</span>
              )}
            </div>
            {errors._variant && (
              <div data-testid="variant-error" className="px-4 py-2 bg-red-50 border-b border-red-200 flex items-center gap-2">
                <AlertCircle size={14} className="text-red-500" /><span className="font-body text-xs text-red-600">{errors._variant}</span>
              </div>
            )}
            <div className="p-4">
              <div className="flex flex-wrap gap-2">
                {variants.map((v) => {
                  ...
                  const isVSoldOut = v.status === "sold_out";
                  return (
                    <button
                      key={v.id}
                      ...
                      onClick={() => {
                        if (!isVSoldOut) {
                          ...
                          setErrors(prev => ({ ...prev, _variant: undefined }));
                        }
                      }}
                      disabled={isVSoldOut}
                      className={`relative px-4 py-3 rounded-xl border-2 transition-all flex flex-col items-center min-w-[90px] ${
                        isVSoldOut
                          ? "border-brand-border bg-brand-bg/50 opacity-50 cursor-not-allowed"
                          : isSelected
                          ? "border-brand-primary bg-brand-primary/5 shadow-sm"
                          : "border-brand-border bg-brand-surface hover:border-brand-text active:scale-95"
                      }`}
                    >
                      {v.image && (
                        <img src={v.image} alt={v.name} className={`w-10 h-10 rounded-lg object-cover mb-1.5 ${isVSoldOut ? "grayscale" : ""}`} />
                      )}
                      <span className={`font-body text-sm font-medium ${isSelected ? "text-brand-primary" : isVSoldOut ? "text-brand-text-secondary line-through" : "text-brand-text"}`}>
                        {v.name}
                      </span>
                      <span className={`font-heading text-sm font-bold mt-0.5 ${isSelected ? "text-brand-primary" : isVSoldOut ? "text-brand-text-secondary" : "text-brand-text"}`}>
                        ${v.price.toFixed(2)}
                      </span>
                      {v.stock >= 0 && v.stock <= 5 && v.status !== "sold_out" && (
                        <span className="font-body text-[9px] text-amber-600 mt-0.5">{v.stock} left</span>
                      )}
                      {isVSoldOut && (
                        <span data-testid={`variant-sold-out-${v.id}`} className="font-body text-[9px] text-red-500 font-semibold uppercase mt-0.5">Sold Out</span>
                      )}
                      {isSelected && !isVSoldOut && (
        ...
        {/* Modifier Groups */}
        {!effectivelySoldOut && !isSoldOut && modifierGroups.length > 0 && (
          <div data-testid="modifier-groups" className="mt-6 space-y-6">
                ...
                <div key={group.id} data-testid={`modifier-group-${group.id}`} className={`bg-brand-surface border rounded-xl overflow-hidden ${hasError ? "border-red-400" : "border-brand-border"}`}>
                  <div className={`px-4 py-3 flex items-center justify-between ${hasError ? "bg-red-50" : "bg-brand-bg/50"}`}>
                    <div className="flex items-center gap-2">
                      ...
                      {isRequired && <span className="px-2 py-0.5 bg-brand-primary text-white text-[9px] font-body font-bold uppercase tracking-wider rounded">Required</span>}
                    </div>
                    <span className="font-body text-[10px] text-brand-text-secondary">{isRequired ? `Select ${group.min_selections || 1}` : group.max_selections > 0 ? `Up to ${group.max_selections}` : "Optional"}</span>
                  </div>
                  {hasError && <div data-testid={`modifier-error-${group.id}`} className="px-4 py-2 bg-red-50 border-b border-red-200 flex items-center gap-2"><AlertCircle size={14} className="text-red-500" /><span className="font-body text-xs text-red-600">{hasError}</span></div>}
                  <div className="divide-y divide-brand-border">
                    {isRequired ? (
                      <RadioGroup value={selected[0] || ""} onValueChange={(val) => handleRequiredSelect(group.id, val)}>
                        {group.options.map((opt) => (
                          <label key={opt.id} data-testid={`modifier-opt-${opt.id}`} className="flex items-center justify-between p-4 cursor-pointer hover:bg-brand-bg/50 transition-colors">
                            <div className="flex items-center gap-3"><RadioGroupItem value={opt.id} id={opt.id} /><span className="font-body text-sm font-medium text-brand-text">{opt.name}</span></div>
                            <span className={`font-heading text-sm font-semibold ${opt.price_adjustment > 0 ? "text-brand-primary" : "text-brand-text-secondary"}`}>{opt.price_adjustment > 0 ? `+$${opt.price_adjustment.toFixed(2)}` : opt.price_adjustment < 0 ? `-$${Math.abs(opt.price_adjustment).toFixed(2)}` : "Included"}</span>
                          </label>
                        ))}
                      </RadioGroup>
                    ) : (
                      group.options.map((opt) => {
                        ...
                        return (
                          <label key={opt.id} data-testid={`modifier-opt-${opt.id}`} className={`flex items-center justify-between p-4 cursor-pointer transition-colors ${atMax ? "opacity-40" : "hover:bg-brand-bg/50"}`}>
                            <div className="flex items-center gap-3"><Checkbox checked={isChecked} onCheckedChange={() => handleOptionalToggle(group.id, opt.id, group.max_selections)} disabled={atMax} className="w-5 h-5 sm:w-4 sm:h-4" /><span className="font-body text-sm font-medium text-brand-text">{opt.name}</span></div>
                            <span className={`font-heading text-sm font-semibold ${opt.price_adjustment > 0 ? "text-brand-primary" : "text-brand-text-secondary"}`}>{opt.price_adjustment > 0 ? `+$${opt.price_adjustment.toFixed(2)}` : "Free"}</span>
                          </label>
                        );
                      })
                    )}
        ...
        {!effectivelySoldOut && !isSoldOut && (
          <div data-testid="product-actions" className="hidden sm:flex items-center gap-4 mt-6 mb-8">
            <div className="inline-flex items-center border border-brand-border rounded-full overflow-hidden">
              <button data-testid="qty-decrease-btn" onClick={() => setQty(q => Math.max(1, q - 1))} className="w-10 h-10 flex items-center justify-center text-brand-text hover:bg-brand-bg"><Minus size={16} /></button>
              <span data-testid="qty-display" className="w-8 text-center font-body text-sm font-medium">{qty}</span>
