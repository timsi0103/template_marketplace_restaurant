...
export default function SignUpPage() {
  const { register, loginWithGoogle, createGuestSession } = useAuth();
        ...
        <div className="bg-brand-surface border border-brand-border rounded-2xl p-8">
          ...
          <div className="mt-6 pt-6 border-t border-brand-border">
            <button
              ...
              onClick={handleGuest}
              disabled={submitting}
              className="w-full py-2.5 bg-brand-bg border border-brand-border rounded-full font-body text-sm font-medium text-brand-text-secondary hover:text-brand-text hover:border-brand-text transition-all"
            >
              Continue as Guest
            </button>
          </div>
